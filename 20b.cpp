...
// EXPLANATION: I watched all of the things that fed into rx. In my input there were 4 of them. I looked at the amount of time between those 4 going high. Then I took the lcm of all 4. That was the right answer, I have to prove why that works, but for now, this source code is good enough
    ...
    for( ; b < 1000001; ++b )
    {
        ...
        while( !e.empty() )
        {
            ...
            for( auto const& d : e.front().second )
            {
                ...
                if( d == "lb" && e.front().first )
                    std::cerr << b << ": " << effect_source.front() << ( e.front().first ? " -high"s : " -low"s ) + " -> " << d << '\n';
                if( d == "rx" && !e.front().first )
                    return b;
